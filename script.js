function init() {
  ...
  const marioObj = {
    ...
    jumpCalc() {
      if (this.upPressed) {
        if (this.jump_val < 0.6) {
          marioObj.y_velocity -= (1 - this.jump_val)
          this.jump_val += 0.1
        } else {
          this.jump_val = 0.6
  ...
  function moveX() {
    if (rightPressed) {
      ...
      if (pipeSideCollision(marioObj.x + marioObj.x_velocity)) marioObj.x_velocity = 0
      ...
      // if (pipeSideCollision(marioObj.x + marioObj.x_velocity)) return
